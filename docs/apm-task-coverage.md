...
| `mise run sync`               | ○        | ○        | ○        | ○           | ○        | Accepts newer upstream package content with `apm install -g --update`, then runs the local rollout                          |
| `mise run sync:stable`        | ○        | ○        | ○        | ○           | ○        | Updates first, then runs the full local rollout without forcing upstream refresh                                            |
...
| `mise run ci`                 | ○        | ○        | ○        | ○           | ○        | Runs `check -> apply -> doctor` for local delivery                                                                          |
| `mise run catalog:stage`      | ○        | ○        | ○        | ○           | ○        | Normalizes `catalog/` into the managed catalog package                                                                      |
| `mise run catalog:tidy`       | ○        | ○        | ○        | ○           | ○        | Runs `catalog:stage`, `validate:catalog`, and `doctor`                                                                      |
| `mise run catalog:register`   | ○        | ○        | ○        | ○           | ○        | Installs a pushed `catalog` ref                                                                                             |
| `mise run catalog:smoke`      | ○        | ○        | ○        | ○           | ○        | Performs a temporary-install smoke test                                                                                     |
