...
| `mise run doctor`                     | 状態確認 | 状態確認 | 状態確認 | 状態確認    | 状態確認 | Checks managed catalog required outputs in deployment targets and fails on missing or wrong-typed paths; other diagnostics remain informational                                                    |
...
| `mise run test`                       | n/a      | n/a      | n/a      | n/a         | n/a      | Runs the Pester and bats suites for `scripts/apm-workspace.*` in parallel; both are also wired as pre-push jobs                                                                                    |
| `mise run test:ps`                    | n/a      | n/a      | n/a      | n/a         | n/a      | Runs the Pester behavioral suite for `apm-workspace.ps1`, fanning each `*.Tests.ps1` file out to its own `pwsh` process (Windows runs a single process); needs a host-installed `pwsh` with Pester |
| `mise run test:sh`                    | n/a      | n/a      | n/a      | n/a         | n/a      | Runs bats unit tests for `scripts/apm-workspace.sh`; parallelizes across files when GNU parallel and `flock`/`shlock` are both present, else runs serially                                         |
...
| `mise run validate:workspace`         | ○        | ○        | ○        | ○           | ○        | Validates workspace inputs only, respecting `APM_WORKSPACE_DIR`; it does not read deployment targets                                                                                               |
...
| `mise run apply:skills:local`         | ○        | n/a      | n/a      | n/a         | n/a      | Quick-syncs catalog/private skills into the Codex skill target and refreshes Claude private skill links; skips the legacy workspace cleanup that full `apply` performs                             |
...
- check parts of `check` / `format:check`: `format:markdown`, `format:markdown:check`, `format:markdown:bold-headings`, `format:markdown:bold-headings:check`, `format:toml`, `format:toml:check`, `format:yaml`, `format:yaml:check`, `lint:yaml`, `lint:frontmatter`, `lint:catalog-leaks`, `validate:workspace`, `validate:catalog`, `smoke:catalog`
- `format:check`, `validate`, `test:ps`, and `test:sh` are aggregate parts but remain visible because CI, the pre-push hook, or the procedures call them directly.
...
| `bundle-catalog` | `bash ./scripts/apm-workspace.sh bundle-catalog [skill-id ...]` | Builds `~/.apm/.catalog-build/catalog` as the catalog package artifact. Also called internally by `prepare:catalog` (via `stage-catalog`/`Invoke-StageCatalog`) and `smoke:catalog` (via `smoke-catalog`/`Invoke-SmokeCatalog`); `install:catalog` does not call it |
