...
### repo-local 移管（撤去ではなく利用継続）

- `agentation` / `agentation-self-driving`: global からは外したが廃止ではない。
  利用リポジトリ: `caad-loca-bff`、`ultra-rss-reader`（各リポジトリの `apm.yml` が正本）。
  Agentation toolbar と実行中ブラウザが必須のため、他リポジトリでは不要。
  移管判断の詳細は [`docs/skill-scope-proposals.md`](skill-scope-proposals.md) を参照。
