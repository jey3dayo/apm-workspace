...
| モーション taste・レビュー・監査           | `emil-design-eng` / `review-animations` / `improve-animations`（emilkowalski/skills） |
| モーション実装スニペット                   | `transitions-dev`（実務で有用だった実績あり）                                         |
...
| コードベース監査→計画（汎用）              | `improve`（shadcn。`improve-animations` の汎用版、併存可）                            |
...
### repo-local で活用中（global には無いが現役）

global の一覧に無くても廃止ではない。各リポジトリの `apm.yml` が正本で、以下は実際に使用中（2026-07-16 時点の `ghq` 配下スキャン）。

| ツール                                                             | 利用リポジトリ                      | 用途                                                                          |
| ------------------------------------------------------------------ | ----------------------------------- | ----------------------------------------------------------------------------- |
| `agentation` / `agentation-self-driving` + `agentation-mcp`（MCP） | `caad-loca-bff`、`ultra-rss-reader` | Agentation toolbar での UI アノテーション連携。toolbar と実行中ブラウザが必須 |
...
移管候補の整理と判断基準は [`docs/skill-scope-proposals.md`](skill-scope-proposals.md) を参照。
...
- 再検討するなら: 2〜3 週間の実運用でモーション系スキルの発火競合・指摘重複を確認する。
