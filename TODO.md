...
- `catalog` は全員向けの管理対象、`optional-skills` は明示的に opt-in する repo-local パッケージ、`apm.yml` は external 依存の宣言として責務を分ける。
...
- 導入先リポジトリで `apm.yml` を更新し、`apm install ... --skill <id>` を実行する。global へ直接追加しない。
- `find-skills` は検索体験・候補説明・導入導線の参考にする。ただし現状は skills.sh / `npx skills` 向けで、社内 catalog、optional package、APM の scope 判定は扱わない。
...
- `[ ]` optional package を GitHub に公開した後、`tech-talks` の `apm.yml` に指定した `google-forms-survey-builder` を lock / deploy まで確定する。
