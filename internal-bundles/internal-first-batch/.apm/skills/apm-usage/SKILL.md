...
Until an internal APM bundle / package reference exists, keep these skills in the rollback/seed lane and describe them as pilot candidates rather than normal global dependencies.

Use `mise run migrate-internal` when you need to stage the first batch into:
...
Use `mise run bundle-internal` when you need a valid APM package artifact at:

```text
~/.apm/.internal-seed/internal-first-batch/
```
...
- `migrate-internal` is the explicit pilot helper for internal bundled skills and seeds `~/.apm/.internal-seed/`
- `bundle-internal` builds a valid internal APM artifact under `~/.apm/.internal-seed/internal-first-batch/`
- `stage-internal` syncs that generated bundle into `~/.apm/internal-bundles/internal-first-batch/` and prints the future `owner/repo/path#branch` upstream ref
- `register-internal` only runs once that staged path is committed and pushed, then installs it by upstream ref
- `smoke-internal` project-scope installs that generated bundle into a temp workspace and checks `.agents/skills/<id>/SKILL.md`
