---
name: skill-creator
description: Use when creating, migrating, or updating a personal skill for this `~/.apm` workspace, or when deciding where a new skill should live. Create skills in `~/.apm/catalog/skills/<id>/`.
---
...
Use it to create or update personal skills that belong to this repository.
...
- A user is unsure whether to work in `catalog/skills/` or `~/.codex/skills/`
...
- Author personal skills only in `~/.apm/catalog/skills/<id>/`
...
3. Create or edit `~/.apm/catalog/skills/<id>/SKILL.md`
...
5. If this skill replaces another one, update workspace references and docs
6. Run `mise run format:markdown:bold-headings`
...
1. Add the personal skill under `~/.apm/catalog/skills/<id>/`
2. Update workspace references to the new id
3. Remove obsolete external refs from `~/.apm/apm.yml` if they are no longer needed
4. Run `mise run format:markdown:bold-headings`
