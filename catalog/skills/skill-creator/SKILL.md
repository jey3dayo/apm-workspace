...
- **Form filling**: Keep a dedicated forms guide when the skill needs one
- **API reference**: Keep a dedicated reference guide when the surface area is large
- **Examples**: Keep a dedicated examples guide for common patterns
```

Codex loads those dedicated guides only when needed.
...
Use docx-js for new documents and keep the detailed guide in a separate reference when needed.

## Editing documents

For simple edits, modify the XML directly.

**For tracked changes**: Keep a dedicated redlining guide when the skill needs it
**For OOXML details**: Keep a dedicated OOXML guide when the skill needs it
```

Codex reads those deeper guides only when the user needs those features.
