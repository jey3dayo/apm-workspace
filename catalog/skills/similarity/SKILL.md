...
- interface の検出には 0.5.0 以降が必要で、0.4.x は type alias だけを検出する。利用できる 0.5.0+ があればそれを使い（別パスのバイナリを直接実行するなど、グローバル状態を変えない経路を優先する）、更新が必要なときはユーザーの確認を取る。どちらも取れなければ interface の重複を手動レビューで補い、使用したバージョンと影響範囲をレポート冒頭に明記する
