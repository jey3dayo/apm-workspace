...
4. 共通化するペアは参照箇所を確認し（Serena の `find_referencing_symbols`、使えなければ `rg` で全参照を列挙）、テストを確保してから 1 ペアずつ抽出する
...
複数ペアが同じ抽出テーマに収束する場合（同型の get / remove 系など）はテーマ単位でまとめ、派生ペアを個別候補として数えない。ユーザーが調査対象を明示している依頼（特定の型群・関数群、型全般の整理など）では、87% 未満でも当該対象を補完調査してよい（低閾値での再実行または手動レビュー）。依頼スコープが一部カテゴリのみの場合、スコープ外の検出結果は参考情報として分離して報告する。判断と計画は生レポートと分けて `tmp/similarity-plan.md` などに書く。既存の成果物ファイルがある場合は上書きせず、トピックや日付を付けた別名にする。
...
- v0.4.x は interface を検出しないバグがある（type alias のみ検出）。対処の優先順位: (1) ユーザーに確認の上で 0.5.0 以降へ更新、(2) グローバル状態を変えない代替（別パスにある 0.5.0+ バイナリの直接実行）、(3) interface の重複を手動レビューで補完。各段の前提が満たせない場合（確認が取れない、代替バイナリが無い等）は次へ進み、選んだ経路・使用バージョン・理由をレポート冒頭に明記する
