...
AWS CLI 実行前に、対象 profile が `~/.aws/config` に定義されていることを確認してください。既知 profile が未定義の場合は、このスキルの `AWS CLI Config Initialization` に従って `credential_process` stanza を追記してから認証を進めます。
...
| `CA-Nic-prd`          | CA-Nic-prd | 開発者用ロールでの本番環境アクセス | `~/.config/perman-aws-vault/CA-Nic-prd`          |
...
# CA-Nic-prdでの作業
perman-aws-vault print -p ~/.config/perman-aws-vault/CA-Nic-prd
aws sts get-caller-identity  # CA-Nic-prdの認証先を確認
```

初回 `perman-aws-vault select` はカレントディレクトリに `.perman-aws-vault` を生成します。profile ごとの `credential_process` で使う場合は、生成ファイルを `~/.config/perman-aws-vault/<profile>/.perman-aws-vault` に配置してから AWS CLI を再試行してください。

`AssumeRoleWithSAML` で `DurationSeconds exceeds the MaxSessionDuration` が出る場合、PERMAN Federation から返る `session_duration` と IAM role の MaxSessionDuration が不一致です。ローカル設定では短縮できないため、PERMAN Service Provider または IAM role の設定を調整してください。
