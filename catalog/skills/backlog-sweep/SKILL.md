...
4. `~/.agents/skills/agmsg/scripts/delivery.sh set both <claude-code|codex> <対象project絶対パス>` で自動配送を入れ、`delivery.sh status <type> <対象project絶対パス>` の `Stop entries` / `PostToolUse entries` を**読み戻す**。**`agent start` より前に置く**——hooks は session 開始時に読まれる。`monitor` だけでは codex が何も pull しない理由と、hooks file を手で書かない理由は `agmsg-delegation` の `references/resident-pool.md` を参照
