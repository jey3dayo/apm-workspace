...
4. `~/.agents/skills/agmsg/scripts/delivery.sh set <mode> <claude-code|codex> <対象project絶対パス>` で自動配送を入れ、`delivery.sh status <type> <対象project絶対パス>` の `Stop entries` / `PostToolUse entries` を**読み戻す**。mode は codex なら `turn`、claude-code なら `both`（`both` は claude-code 専用）。**`agent start` より前に置く**——hooks は session 開始時に読まれる。モードの選び方と hooks file を手で書かない理由は `agmsg-delegation` の `references/resident-pool.md` を参照
5. `herdr agent start <worker_name> --kind <claude|codex> --pane <新 pane_id> -- <ネイティブ引数...>` で起動する。**`pane run` は使わない** — `agent start` は Herdr が同じ pane で当該 agent を検出し入力受付可能と判断するまで返らないので、未起動を「起動した」と報告する事故が構造的に起きない。モデル指定などは `--` の後にそのまま渡す（例: `--kind codex -- -m gpt-5.6-luna`、`--kind claude -- --model opus`）。startup 中に承認待ちになると即 `agent_not_ready` が返るので、その場合は `herdr agent read <worker_name>` で画面を見てから判断する
6. `herdr agent get <worker_name>` で lifecycle 状態を、`herdr pane process-info --pane <新 pane_id>` で cwd を**読み戻す**。読み戻していない状態やプロセス名を報告に書かない
7. 報告には pane_id + workspace label + 絶対 cwd + 実際に読み戻した状態を併記する
8. 起動後の指示出しは `herdr agent prompt <worker_name> "..." --wait`（settled 状態まで待つ）。承認待ちを掴みたいときだけ `herdr agent wait <worker_name> --until blocked`
