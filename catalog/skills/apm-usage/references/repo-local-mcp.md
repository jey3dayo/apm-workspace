...
| ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Next.js, React, Vite, or another browser-served frontend                 | Keep common web skills global. Prefer `claude-in-chrome`/Codex Chrome addon first; add repo-local or on-demand `chrome-devtools` only for DevTools-specific depth (Lighthouse, performance trace, heap snapshot), project login/session state, local runtime coupling, or repeatable browser verification. |
| Tauri app with `src-tauri`                                               | `tauri-mcp-server`; add `chrome-devtools` only if DevTools-specific depth is needed on top of `claude-in-chrome`/Codex Chrome addon                                                                                                                                                                        |
...
| One-off visual inspection or desktop automation                          | Screen automation MCPs on demand; OS-level screenshots use the `screenshot` skill                                                                                                                                                                                                                          |

## chrome-devtools Exception Documentation
