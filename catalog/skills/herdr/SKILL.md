...
## Runtime workspace, tab, and pane control

既存の Herdr セッションで特定 workspace / tab / pane を操作するときは、TUI へキー入力を推測で送らず socket API helper を使う。

1. `herdr status` で server の稼働状態を確認する。
2. `herdr workspace list`、`herdr tab list --workspace <workspace_id>`、`herdr pane list --workspace <workspace_id>` の順で対象 ID を特定する。ユーザーが「3 タブ目」のように指定した場合は workspace 内の label / 表示順を照合し、raw `number` が workspace 内の連番だと仮定しない。
3. `herdr pane process-info --pane <pane_id>` で foreground process を確認する。shell が待機中の pane にだけコマンドを起動し、agent や別 TUI が動作中なら上書きせず、別 pane の選択・作成またはユーザー確認へ切り替える。
4. `herdr pane run <pane_id> "cd <absolute-cwd> && <command>"` で対象 pane にコマンドを起動する。`hunk diff` や `lazygit` などの interactive TUI も agent 自身の PTY ではなく、ユーザーが見る Herdr pane で起動する。
5. 必要なら `herdr tab focus <tab_id>` で対象 tab を表示する。
6. `herdr pane process-info --pane <pane_id>` を再実行し、期待した foreground process と cwd になったことを確認する。

例: 指定した worktree の Hunk review を対象 tab で開始する。

```bash
herdr pane run <pane_id> "cd /absolute/path/to/worktree && hunk diff"
