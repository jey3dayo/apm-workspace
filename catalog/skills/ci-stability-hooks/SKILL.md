...
- Predict the failures no hook can catch: ones caused by the workflow's own dependencies rather than the code being pushed. Deprecated action majors and retired runner Node versions fail CI without any code change. Two cheap detectors, run once during this review and reported as advice: read the annotations of the latest run on the default branch (`gh run view <id>`) for deprecation warnings, and, when the repo uses pnpm 11.16.0 or later, run `pnpm outdated --include-github-actions` and report the outdated `uses:` entries. Both belong in this review, not in `pre-push`: the pnpm check runs `git ls-remote` against every referenced action repository and fails on any outdated dependency, not only on ones that break CI.
- Suggest `update.githubActions: true` in `pnpm-workspace.yaml` only when the repo has no Renovate or Dependabot covering workflow files, and say what it changes: `pnpm update` then rewrites `uses:` refs to exact commit hashes with the tag kept as a comment. That is a policy change to the workflow files, so leave the decision to the user instead of enabling it as part of hook work. Actions on GitHub Enterprise Server need `update.githubActionsServer` (pnpm 11.17.0+).

Completion condition: local push checks catch normal CI failures, stale actions are reported, and intentional gaps are reported.
