...
For the current personal workspace, create tasks under team `JEY` and route them to a project. `finance-ops` is a project, not a team/workspace.
...
| ------------ | ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| finance-ops  | お金周り・決済・サブスク・固定費・ポイ活・家計・サービス利用 | 支払い、請求、決済、サブスク、解約、ポイント、固定費、d払い、クレカ、引き落とし、ギフトコード、特典、期限 |
| labs         | 個人学習・調査・技術検証・研究・個人開発実験                 | 調べる、検証、学習、試す、リサーチ、PoC、実験、ドキュメント読む                                           |
...
2. 複数に該当 / 判断できない → 候補プロジェクト名を挙げてユーザーに確認
