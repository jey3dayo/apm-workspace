---
name: "gh-fix-ci"
description: "Use when a user asks to debug or fix failing GitHub PR checks that run in GitHub Actions. Use `gh` for PR metadata, patch context, and Actions check and log inspection before implementing any approved fix."
metadata:
  author: OpenAI
  source: openai/plugins plugins/github/skills/gh-fix-ci@11c74d6 (retired upstream in 33bd952; salvaged 2026-08-28, connector references replaced with gh CLI)
---
...
- Use `gh` for PR metadata, changed files, review context, and GitHub Actions checks and logs.
...
2. Resolve the PR.
   ...
   - When repo and PR are known, fetch PR metadata and patch context with `gh pr view` / `gh pr diff`.
